...
import de.druz.game.gdxgame.components.Sprite;
...
public class SpriteRenderSystem extends EntitySystem {
	...
	public SpriteRenderSystem(OrthographicCamera camera) {
		super(Aspect.getAspectForAll(Transform.class, Sprite.class));
		this.camera = camera;
	}
	...
	protected void process(Entity e) {
		if(tm.has(e)) {
		Transform transform = tm.getSafe(e);
		Sprite sprite = sm.get(e);

		AtlasRegion spriteRegion = regionsByEntity.get(e.getId());
		batch.setColor(sprite.r, sprite.g, sprite.b, sprite.a);

		float posX = transform.getX() - (spriteRegion.getRegionWidth() / 2 * sprite.scaleX);
		float posY = transform.getY() - (spriteRegion.getRegionHeight() / 2 * sprite.scaleX);
